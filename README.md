...
## 🕹️ Interactive App Note (Important for Reviewers)

This Streamlit application is **interactive** ✅  
Many results are generated only after clicking the buttons (just like running notebook cells).

### ✅ Artifacts generated by buttons
When you click the buttons in the app, it creates artifacts such as:

- **STAC cache**: `data/cairo_stac_cache.npz` (created in **B1**)  
- **Training dataset** (created in **B3**)  
- **Best trained model** (selected in **B4**)  
- **ONNX exported model**: `models/urban_growth_best_model.onnx` (created in **B5**)  

✅ If a page says “missing data / missing model”, simply follow the navigation order and click the button in the previous step.
